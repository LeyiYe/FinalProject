...
from pysph.sph.solid_mech.basic import get_particle_array_elastic_dynamics, ElasticSolidsScheme
...
class GraspDeformableBlock(Application):
    def initialize(self):
        ...
        # Lower elastic modulus so block deforms more and generates grip force
        self.E_block = 1e6  # Pa
        self.nu      = 0.49
        self.c0      = 50.0
    ...
    def create_scheme(self):
        scheme = ElasticSolidsScheme(
            elastic_solids=['block'], solids=['platform','gripper1','gripper2'],
            dim=self.dim, artificial_stress_eps=0.5, xsph_eps=0.5
        )
    ...
    def create_equations(self):
        ...
        if self.gravity_enabled:
            eqns.append(Group(equations=[
                BodyForce(dest='block', sources=[], fx=0, fy=0, fz=-9.81)
            ], real=False))
        eqns.append(Group(equations=[
        ...
        eqns.append(Group(equations=[
            RigidBodyWallCollision('block',['platform'], kn=1e5, mu=2.0, en=0.2),
            RigidBodyWallCollision('block',['gripper1'], kn=1e5, mu=2.0, en=0.2),
            RigidBodyWallCollision('block',['gripper2'], kn=1e5, mu=2.0, en=0.2),
        ], real=False, update_nnps=True))
        eqns.append(Group(equations=[
            ForceToAcceleration(dest='block')
        ], real=False))
        return eqns
    ...
    def post_step(self, solver):
        ...
        # deeper overlap for stronger grip
        target = -half_b - half_g + 0.02  # 2 cm
