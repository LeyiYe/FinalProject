...
class GraspDeformableBlock(Application):
    ...
    def create_particles(self):
        ...
        # --------------------------------------------------------------------
        # 2) Rigid “walls”: platform + two gripper jaws
        # --------------------------------------------------------------------
        ...
        # store handles for post_step
        self.gr1, self.gr2 = gr1, gr2

        return [block, platform, gr1, gr2]

    def create_scheme(self):
        # only the block is an elastic solid
    ...
    def post_step(self, solver):
        # open/close + manual movement of the jaws
        g1, g2 = self.gr1, self.gr2
        dt = solver.dt

        half_b = 0.5*self.block_size[0]
        half_g = 0.5*self.gripper_size[0]
        target = -half_b - half_g + 0.005   # 5 mm beyond contact
        ...
        # **manually advance** jaw positions by their velocities
        for g in (g1, g2):
            g.x  += g.u * dt
            g.y  += g.v * dt
            g.z  += g.w * dt
